<template>
  <el-aside class="input-section">
    <div class="input-card">
      ...
      <!-- 建议区域 -->
      <SuggestionArea @fill-example="handleFillExample" />
    </div>
  </el-aside>
</template>
...
import SuggestionArea from './SuggestionArea.vue'
...
export default {
  ...
  emits: ['generate', 'update-form', 'fill-example'],
  methods: {
    handleGenerate() {
      this.$emit('generate')
    },
    ...
    handleFillExample(type) {
      this.$emit('fill-example', type)
    }
  }
}
</script>
...
.generate-btn {
  ...
  font-weight: 600;
  border-radius: 12px;
  background: linear-gradient(135deg, #d80b0b, #ff8e8e);
  border: none;
