...
export default {
  ...
  setup() {
    ...
    const restoreHistory = (historyData) => {
      // 还原表单数据
      Object.assign(productForm, historyData.productForm)
      
      // 还原生成的内容
      Object.assign(generatedContent, historyData.generatedContent)
    }

    // 保存当前状态到历史记录
    const saveToHistory = () => {
      if (historySidebar.value && productForm.name && generatedContent.note) {
        historySidebar.value.addHistory({
      ...
      switch (type) {
        ...
        case 'iphone':
          Object.assign(productForm, {
            ...
            category: 'tech',
            price: '9999-12999元',
            description: 'A18 Pro芯片性能强劲，钛金属机身轻盈耐用，4800万像素主摄支持5倍光学变焦。120Hz ProMotion显示屏，续航能力出色，支持Action Button和USB-C接口。',
            targetUser: '追求高端科技体验的用户',
            scenario: '日常使用、摄影创作、商务办公、游戏娱乐'
          })
          break
      }
    }
